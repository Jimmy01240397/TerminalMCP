...
- **Bearer-token isolation.** `list_sessions` only shows your own sessions — you can't see or steal another agent's. If you expect a session to be there and it isn't, you may be talking to the server with a different token than you think. If the server runs without a whitelist (the default), missing/empty Bearer collapses everyone into one shared anonymous pool — set a token to keep your sessions to yourself.
